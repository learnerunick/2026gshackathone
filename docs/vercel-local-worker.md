...
| `api/gateway.mjs`, `deploy/gateway.mjs` | 로그인, 세션·CSRF 검증, Mac 요청 서명, 스트리밍 |
| `deploy/routes.json` | Mac과 Vercel이 공유하는 원격 허용 경로 |
| `apps/backend/remote_bridge.py` | 기존 로컬 API에 연결하는 별도 인증 서버 |
| `deploy/vercel/` | 원격 빌드에만 추가하는 로그인·연결 표시·분할 업로드 |
...
| `BOCA_SESSION_SECRET` | 자동 생성된 로그인 세션 키. Vercel에서 사용 |
| `BOCA_DASHBOARD_PASSWORD` | 자동 생성된 대시보드 접속 암호. Vercel에서 사용 |

세 비밀값은 서로 다른 값으로 생성된다. 서버 환경변수이므로 `NEXT_PUBLIC_` 등을 붙여 브라우저에 공개하지 않는다. 실제 값 대신 `.env.example`만 커밋한다.
...
터널의 서비스 주소는 반드시 `http://127.0.0.1:8787`을 사용한다. 기존 로컬 UI 포트 `8765`에는 원격 로그인·요청 서명 검증이 없으므로 터널을 직접 연결하지 않는다. 이 브리지는 서명 없는 요청을 거부한다. Cloudflare Access를 별도로 켠 경우 이 구현에는 Access 서비스 토큰 전달 기능이 없으므로 해당 인증 설정도 함께 맞춰야 한다.
...
빌드·함수 설정은 `vercel.json`에 포함되어 있다. `.env.remote`의 환경변수 다섯 개를 Vercel 프로젝트의 **Production 환경변수**에 등록하고 배포한다. 주소를 바꾸거나 환경변수를 변경한 경우 다시 배포한다. `BOCA_PUBLIC_ORIGIN`과 다른 별칭·Preview URL에서는 변경 요청이 차단된다. 운영 환경변수는 Preview에 복사하지 않는다. 별도 Preview가 필요하면 별도 Mac 테스트 데이터와 주소·키를 사용한다.

로그인 화면에서 `BOCA_DASHBOARD_PASSWORD` 값을 입력한다. 현재 Mac에 있는 콘텐츠와 실행 상태가 표시되어야 한다. 첫 확인은 데이터 조회와 영상 재생으로 한다. 시작·재개 버튼은 실제 Mac 제작 작업자를 깨우므로 생성할 의도가 있을 때 사용한다.
...
- **접속:** 공용 운영 암호와 8시간 `HttpOnly` 세션을 사용한다. 사용자별 계정·권한·감사 기능은 포함하지 않는다. 키 유출 시 세션 키와 접속 암호를 교체하고 다시 배포한다. 브리지 키를 바꾸면 Mac과 Vercel을 함께 갱신한다.
...
- **중단 표시:** Mac 서버·터널에 연결되지 않으면 연결 오류를 표시한다. 로그인 갱신은 새 창에서 할 수 있어 기존 탭의 미저장 입력을 보존한다. 연결 상태는 Mac API 도달 여부이며 Codex의 로그인·사용 한도까지 보장하지 않는다.
...
로컬 화면 검증용 `npm run dev:remote`도 제공한다. 서버 환경에 다섯 환경변수와 `BOCA_ALLOW_LOCAL_DEVELOPMENT=1`을 넣으면 loopback HTTP 주소만 허용한다. 기본 화면 포트는 `8788`이다. 이 스크립트는 기존 Mac 서버나 브리지를 자동 시작하지 않는다. Vercel에서는 개발 예외를 사용할 수 없다.
