...
function renderFeedCell(content) {
  ...
  if (/\.(mp4|webm|mov)$/i.test(card?.media || ""))
    return `<div class="instagram-cell instagram-video-cell">${media(card, { interactive: true })}<span class="cell-status">${esc(labels[content.status] || content.status)}</span><button type="button" class="feed-video-detail" data-action="open-content" data-id="${esc(content.id)}" aria-label="${esc(content.payload.title)} 문안과 검수 보기">문안 · 검수 보기 ${icon("arrow")}</button></div>`;
