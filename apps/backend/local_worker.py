...
class LocalProductionWorker:
    ...
    def _prompt(self, run):
        return ("You are the authorized runtime coordinator for this user's Boca content production request, "
                ...
                "Complete at most one content cycle in this invocation; the local runner schedules the next. "
    ...
    def _finished(self, run_id, returncode, error):
    ...
    def _run_once(self, run, lock):
        ...
        self._status(status="starting", run_id=run["id"], thread_id=None, message="AI 제작 작업자를 시작하고 있습니다.")
        with self.store.db() as db:
        ...
        with os.fdopen(log_fd, "w") as log, os.fdopen(err_fd, "w") as stderr:
            ...
            def read_events():
                for line in process.stdout:
                    ...
                    if event.get("type") in ("error", "turn.failed"):
                        ...
                        if any(word in detail for word in ("usage limit", "rate limit", "quota")):
                            errors.append("Codex 사용 한도에 도달했습니다. 한도가 회복된 뒤 재개해 주세요.")
                        elif any(word in detail for word in ("unauthorized", "authentication", "login")):
                            errors.append("Codex 로그인을 확인한 뒤 재개해 주세요.")
                        else:
                            errors.append("Codex 실행 오류로 제작을 중단했습니다. 작업자 진단 기록을 확인해 주세요.")
            ...
            while process.poll() is None:
                ...
                if time.monotonic() - started > 2700:
                    errors.append("제작 작업자의 최대 실행 시간(45분)을 초과했습니다. 저장된 결과를 확인해 주세요.")
                    self._terminate_process(process)
                    break
        ...
        self._finished(run["id"], process.returncode, errors[-1] if errors else None)
        with self.store.db() as db:
        ...
        current, cycle = self._snapshot()
        if current and current["id"] == run["id"] and current["status"] == "running" and not progress and not (cycle and cycle["status"] == "running"):
                ...
                while not self.stop.is_set():
                    ...
                    try:
                        ...
                        if self._runnable(run, cycle):
                            ...
                            self._status(status="idle", run_id=run["id"] if run else None,
                                         message="제작 요청 또는 진행 중인 단계의 결과를 기다리고 있습니다.")
